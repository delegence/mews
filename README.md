...
One movable **Hub** owns canonical Agent definitions, model credentials, Sessions, and history. Every machine—including the machine running Hub—is an ordinary **Host** that provides tools and a filesystem. The built-in `mews` Harness is deliberately minimal: a model loop with `read`, `write`, `edit`, and `bash`.
...
- `codex` and `claude` are managed ACP Harness recipes. Run `mews harnesses setup codex` or `mews harnesses setup claude` on each Host where they should execute.
...
The built-in `mews` Harness discovers Agent Skills from `<MEWS_HOME>/skills/` and ancestor
`.agents/skills/`, and prompt templates from `<MEWS_HOME>/prompts/` and ancestor
`.agents/prompts/`. Invoke a prompt with `/name arguments`. Unsandboxed runtime
...
Harnesses receive run-scoped extension tools through MCP when they support it, but do not yet
receive the Skill inventory or lifecycle hooks.
